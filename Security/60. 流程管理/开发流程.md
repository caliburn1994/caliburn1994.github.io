...
集成环境：开发阶段的代码在被合并之后，会自动部署到一个公司内部的开发环境。当代码被前后端的开发者认为没有问题后，就会交付给测试团队，此时将会部署到测试环境上。

测试环境：测试团队进行测试时，将有可能发现一些漏洞，此时将需要和团队 Leader 和开发人员进行协调。Leader 将会最终给出一个答案，是否可以将测试代码部署。
...
生产环境（production）：正式环境。正式部署需要采用蓝绿部署（Blue-green deployment）。但在这个基础之下有：
...
- 安全监控
  ...
  - 项目
    ...
    - 依赖库的脆弱性以及合法性
    - 代码脆弱性：SATS 等工具
    - 机密性：不提交带有密码和 token 的 commit 
    - 法律法规：个人信息保护法（不使用客户数据做测试数据）
